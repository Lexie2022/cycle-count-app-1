...
def create_inventory_report(df, accuracy, shortage_df, overage_df):
    """生成盘点 PDF 报告（包含准确率、Top表格、差异图）"""
    
    # 1️⃣ 生成差异图表
    fig, ax = plt.subplots(figsize=(6, 4))
    df.plot(kind='bar', x='SKU', y='Variance', ax=ax, legend=False, color='steelblue')
    ax.set_title("Inventory Variance Distribution", fontsize=14)
    ax.set_xlabel("SKU")
    ...
    # 报告标题
    story.append(Paragraph("<b>📦 Inventory Cycle Count Report</b>", styles["Title"]))
    ...
    shortage_data = [["SKU", "SystemQty", "CountedQty", "Variance"]] + shortage_df.values.tolist()
